...
export class ContourMapCtrl extends MetricsPanelCtrl {
  ...
  /** @ngInject **/
    constructor($scope, $injector) {
        ...
        ctrl.panel.tileServer = _.find(TileServers, function (ts: any) { return ts.Name == ctrl.panel.mapBackground;});
        ctrl.panel.maxZoom = ctrl.panel.tileServer.Options.options.maxZoom;
    ...
    onDataRecieved(data: any) {
        ...
        if (ctrl.metaData == null) {
            ctrl.datasource.getMetaData(data.map(function(x: any) { return "'" + x.pointtag + "'"; }).join(',')).then(function (metaData: any) {
                ctrl.metaData = JSON.parse(metaData.data);
                _.each(ctrl.metaData, function (element: any, index: any, list: any) {
                    var datam = _.find(data, function (y: any) {
            ...
            _.each(ctrl.metaData, function (element: any, index: any, list: any) {
                var datam = _.find(data, function(x: any) {
    ...
    createMap() {
        ...
        if (ctrl.$scope.mapContainer == null) {
            ...
            ctrl.$scope.mapContainer.on('zoomend', function (event: Event) {
            ...
            ctrl.$scope.mapContainer.on('moveend', function (event: Event) {
                ctrl.panel.zoomLevel = ctrl.$scope.mapContainer.getZoom();
    ...
    getRange(min: any, max: any, step: any) {
        step = step || 1;
        var input = [];
